...
The identity card is regenerated by GitHub Actions from the authenticated account inventory. It deduplicates every repository accessible through personal ownership, direct collaboration, and organization membership—including public, private, and internal repositories—while publishing aggregate counts only. Required-owner validation and a verified repository-count floor prevent partially authorized credentials from silently lowering the public totals.

Private repository names, owner names beyond the intentionally linked public organizations, branch names, commit messages, URLs, descriptions, and per-repository statistics are never written to generated telemetry. See the [generator documentation](docs/profile-generator.md) for coverage semantics, authentication setup, and failure guarantees.
