...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_workflow_uses_current_official_actions_and_environment_only_secrets(self) -> None:
        ...
        generation_step = workflow.index("- name: Generate authenticated aggregate telemetry")
    ...
    def test_workflow_preserves_bootstrap_assets_until_primary_credentials_exist(self) -> None:
        workflow = WORKFLOW.read_text(encoding="utf-8")

        self.assertIn(
            "TELEMETRY_CONFIGURED: ${{ secrets.PROFILE_STATS_TOKEN != '' || secrets.PROFILE_STATS_TOKENS != '' }}",
            workflow,
        )
        self.assertIn("- name: Preserve bootstrap assets without telemetry credentials", workflow)
        self.assertIn("if: env.TELEMETRY_CONFIGURED != 'true'", workflow)
        self.assertIn(
            "No primary telemetry credential is installed; preserving current generated assets.",
            workflow,
        )

        authenticated_steps = (
            "Generate authenticated aggregate telemetry",
        ...
        for step_name in authenticated_steps:
            ...
            self.assertIn("if: env.TELEMETRY_CONFIGURED == 'true'", following)

        tests_position = workflow.index("python -m unittest discover -s tests -v")
        guard_position = workflow.index("TELEMETRY_CONFIGURED:")
